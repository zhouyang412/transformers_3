...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # x --> (batch, seq_length, n_state*3)
        x = self.c_attn(x)
        # query, key, value --> (batch, seq_length, n_state)
        query, key, value = x.split(self.split_size, dim=2)
        # query, value --> (batch, n_head, seq_length, n_state/n_head)
        # key --> (batch, n_head, n_state/n_head, seq_length)
        ...
        if use_cache is True:
            # (2, batch, n_head, seq_length, n_state/n_head)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    Return:
        ...
        # 计算方式见200行
        # 存放之前状态的每一层的 key, value --> (2, batch, n_head, seq_length, n_state/n_head)
        if past is None:
            past_length = 0
