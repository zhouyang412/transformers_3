...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertLMPredictionHead(config)
        self.seq_relationship = nn.Linear(config.hidden_size, 14)
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    Examples::
        ...
        if labels is not None and next_sentence_label is not None:
            ...
            loss_fct_mlm = CrossEntropyLoss(ignore_index=-1)
            masked_lm_loss = loss_fct_mlm(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
            next_sentence_loss = loss_fct(seq_relationship_score.view(-1, 14), next_sentence_label.view(-1))
            total_loss = masked_lm_loss + 0.5 * next_sentence_loss
            #(total_loss,) + outputs
            return total_loss
