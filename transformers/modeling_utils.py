...
class Conv1D(nn.Module):
    ...
    def forward(self, x):
        # x.size()[:-1] --> (bahch, seq_length)
        # size_out --> (batch, seq_length, nf)
        size_out = x.size()[:-1] + (self.nf,)
        # weight --> (nx, nf); x.view(-1, x.size(-1)) --> (batch * seq_length, nx)
        # x.view(-1, x.size(-1)), self.weight --> (batch * seq_length, nf)
        # torch.addmm(b, x, y) --> x * y(矩阵相乘)  + b
